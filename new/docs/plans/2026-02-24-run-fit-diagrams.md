# Run-Fit Diagram Generation — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Generate 12 landscape .docx files (one per defensive front) showing all 11 defenders' run-fit gap responsibilities on every diagram, with base alignments vs 9 formations and legal stunts vs 2×2.

**Architecture:** Single Python script `create_runfit_diagrams.py`. Data-driven: fronts, gap fits, stunts, and formations defined as dicts. Generic render function draws any combination. Each front produces one .docx in `diagram_docs/runfit/`.

**Tech Stack:** Python 3, matplotlib (diagrams), python-docx (landscape .docx), PNG at 150dpi.
...
**Files:**
- Create: `/home/ksc4130/src/defensive_playbook/create_runfit_diagrams.py`

**Step 1: Create the script with imports, color constants, Y-depth constants, and output directories**
...
Run: `cd /home/ksc4130/src/defensive_playbook && python3 create_runfit_diagrams.py`
Expected: No output, no error.

---

### Task 2: Drawing Primitives

**Files:**
- Modify: `/home/ksc4130/src/defensive_playbook/create_runfit_diagrams.py`

**Step 1: Add drawing primitive functions**
...
### Task 3: Offensive Formation Data and Drawing

**Files:**
- Modify: `/home/ksc4130/src/defensive_playbook/create_runfit_diagrams.py`

**Step 1: Add OL drawing, QB, RB, WR primitives and all 9 formations**
...
def draw_gap_labels(ax, te_field=False, te_bnd=False):
    ...
    # Base gaps: C(outside OT) B(OG-OT) A(C-OG) A(C-OG) B(OG-OT) C(outside OT)
    gaps = [
        (5.5, "C"), (3, "B"), (1, "A"), (-1, "A"), (-3, "B"), (-5.5, "C")
    ]
    if te_field:
        gaps.append((7.5, "D"))  # outside TE gap (or relabel C→B, etc.)
        # Shift: with TE field, C gap is between OT and TE (5,6) = x~5
        gaps = [
            (7.5, "D"), (5, "C"), (3, "B"), (1, "A"),
            (-1, "A"), (-3, "B"), (-5.5, "C")
        ]
    if te_bnd:
        if te_field:  # both TEs
            gaps = [
                (7.5, "D"), (5, "C"), (3, "B"), (1, "A"),
                (-1, "A"), (-3, "B"), (-5, "C"), (-7.5, "D")
            ]
...
### Task 4: Defensive Front Data with Gap Fits

**Files:**
- Modify: `/home/ksc4130/src/defensive_playbook/create_runfit_diagrams.py`

**Step 1: Define all 12 fronts with positions AND gap fit targets**

Each player entry: `(x, y, label, color, gap_target_x)` where `gap_target_x` is the X coordinate on the LOS that their dotted line points to. For contain/force players, the target is outside the OL.

```python
# Gap target X coordinates (at Y_LOS level)
# These correspond to gaps between OL at x positions: -4, -2, 0, 2, 4
# A gaps: ±1, B gaps: ±3, C gaps: ±5.5, Force/contain: ±7 or wider
GAP_A_FIELD = 1.0
GAP_A_BND = -1.0
GAP_B_FIELD = 3.0
GAP_B_BND = -3.0
...
# Front definitions: each player = (x, y, label, color, gap_x)
# gap_x = where their dotted run-fit line points on the LOS

def _std_secondary():
    """FC, BC, FS, D — standard positions with run-fit targets."""
    return [
...
FRONTS = {
    "SHADE": {
        "dl": [
            ...
            (2.5, Y_DL, "T", C_NAVY, GAP_B_FIELD),  # 3-tech: B gap
            (-1.5, Y_DL, "N", C_NAVY, GAP_A_BND),    # 2i: A gap
            (-4.7, Y_DL, "E", C_NAVY, CONTAIN_BND),
        ],
        "lbs": [
            (1.5, Y_LB, "M", C_COLUMBIA, GAP_A_FIELD),   # open A field
            (-1.5, Y_LB, "W", C_COLUMBIA, GAP_B_BND),     # open B bnd
            (7.5, Y_APEX, "B", C_COLUMBIA, FORCE_FIELD),   # field force
        ],
    ...
    "UNDER": {
        "dl": [
            (4.7, Y_DL, "A", C_NAVY, CONTAIN_FIELD),
            (1.5, Y_DL, "T", C_NAVY, GAP_A_FIELD),    # 2i: A gap
            (-2.5, Y_DL, "N", C_NAVY, GAP_B_BND),     # 3-tech: B gap
            (-4.7, Y_DL, "E", C_NAVY, CONTAIN_BND),
        ],
        "lbs": [
            (1.5, Y_LB, "M", C_COLUMBIA, GAP_B_FIELD),   # open B field
            (-1.5, Y_LB, "W", C_COLUMBIA, GAP_A_BND),     # open A bnd
    ...
    "EYES": {
        ...
        "lbs": [
            (1.5, Y_LB, "M", C_COLUMBIA, GAP_B_FIELD),   # open B
            (-1.5, Y_LB, "W", C_COLUMBIA, GAP_B_BND),     # open B
            (7.5, Y_APEX, "B", C_COLUMBIA, FORCE_FIELD),
        ],
        "secondary": _std_secondary(),
    },
    "WIDE": {
        ...
        "lbs": [
            (1.5, Y_LB, "M", C_COLUMBIA, GAP_A_FIELD),   # open A
            (-1.5, Y_LB, "W", C_COLUMBIA, GAP_A_BND),     # open A
            (7.5, Y_APEX, "B", C_COLUMBIA, FORCE_FIELD),
        ],
        "secondary": _std_secondary(),
    },
    "DEUCES": {
        "dl": [
            (4.7, Y_DL, "A", C_NAVY, CONTAIN_FIELD),
            (2.0, Y_DL, "T", C_NAVY, GAP_B_FIELD),    # 2-tech: read/react
            (-2.0, Y_DL, "N", C_NAVY, GAP_B_BND),     # 2-tech: read/react
            (-4.7, Y_DL, "E", C_NAVY, CONTAIN_BND),
        ],
        "lbs": [
            (1.5, Y_LB, "M", C_COLUMBIA, GAP_A_FIELD),  # reacts to T
            (-1.5, Y_LB, "W", C_COLUMBIA, GAP_A_BND),    # reacts to N
            (7.5, Y_APEX, "B", C_COLUMBIA, FORCE_FIELD),
        ],
        "secondary": _std_secondary(),
    },
    "GRIZZLY": {
        "dl": [
            (3.5, Y_DL, "A", C_NAVY, GAP_B_FIELD),    # 4i: B gap
            (1.5, Y_DL, "T", C_NAVY, GAP_A_FIELD),     # 2i: A gap
            (-1.5, Y_DL, "N", C_NAVY, GAP_A_BND),      # 2i: A gap
            (-3.5, Y_DL, "E", C_NAVY, GAP_B_BND),      # 4i: B gap
        ],
        "lbs": [
            (0, Y_LB, "M", C_COLUMBIA, GAP_C_FIELD),  # 10-tech, C gap
            (-6.5, Y_OLB, "W", C_COLUMBIA, CONTAIN_BND),  # OLB bnd
            (6.5, Y_OLB, "B", C_COLUMBIA, CONTAIN_FIELD), # OLB field
        ],
        "secondary": _std_secondary(),
    },
    "BOSS": {
        "dl": [
            (4.7, Y_DL, "A", C_NAVY, CONTAIN_FIELD),
            (2.5, Y_DL, "T", C_NAVY, GAP_B_FIELD),    # 3(F)
            (0.5, Y_DL, "N", C_NAVY, GAP_A_FIELD),     # 1(F)
            (-4.7, Y_DL, "E", C_NAVY, CONTAIN_BND),
        ],
        "lbs": [
            (-0.5, Y_LB, "M", C_COLUMBIA, GAP_A_BND),   # A gap bnd
            (-2.5, Y_LB, "W", C_COLUMBIA, GAP_B_BND),    # B gap bnd
            (7.5, Y_APEX, "B", C_COLUMBIA, FORCE_FIELD),
        ],
        "secondary": _std_secondary(),
    },
    "BOSS UNDER": {
        "dl": [
            (4.7, Y_DL, "A", C_NAVY, CONTAIN_FIELD),
            (-0.5, Y_DL, "T", C_NAVY, GAP_A_BND),     # 1(B)
            (-2.5, Y_DL, "N", C_NAVY, GAP_B_BND),     # 3(B)
            (-4.7, Y_DL, "E", C_NAVY, CONTAIN_BND),
        ],
        "lbs": [
            (0.5, Y_LB, "M", C_COLUMBIA, GAP_A_FIELD),   # A gap field
            (2.5, Y_LB, "W", C_COLUMBIA, GAP_B_FIELD),    # B gap field
            (7.5, Y_APEX, "B", C_COLUMBIA, FORCE_FIELD),
        ],
        "secondary": _std_secondary(),
    },
    ...
    "MINT": {
        "dl": [
            (3.5, Y_DL, "A", C_NAVY, GAP_B_FIELD),    # 4i
            (0, Y_DL, "T", C_NAVY, GAP_A_FIELD),        # 0-tech
            (-3.5, Y_DL, "N", C_NAVY, GAP_B_BND),       # 4i
        ],
        "lbs": [
            (1.5, Y_LB, "M", C_COLUMBIA, GAP_A_FIELD),   # A gap
            (-1.5, Y_LB, "W", C_COLUMBIA, GAP_A_BND),     # A gap
            (7.5, Y_OLB, "B", C_COLUMBIA, CONTAIN_FIELD), # OLB field
            (-7.5, Y_OLB, "E", C_COLUMBIA, CONTAIN_BND),  # OLB bnd
        ],
        "secondary": _std_secondary(),
    },
    "ACE": {
        "dl": [
            (4.0, Y_DL, "A", C_NAVY, GAP_B_FIELD),    # 4-tech 2-gap
            (0, Y_DL, "T", C_NAVY, GAP_A_FIELD),        # 0-tech 2-gap
            (-4.0, Y_DL, "N", C_NAVY, GAP_B_BND),       # 4-tech 2-gap
        ],
        "lbs": [
    ...
    "JET": {
        "dl": [
            (4.7, Y_DL, "A", C_NAVY, CONTAIN_FIELD),  # 5-tech contain
            (0, Y_DL, "T", C_NAVY, GAP_A_FIELD),        # 0-tech 2-gap
            (-4.7, Y_DL, "N", C_NAVY, CONTAIN_BND),     # 5-tech contain
        ],
        "lbs": [
            (1.5, Y_LB, "M", C_COLUMBIA, GAP_B_FIELD),   # B gap field
            (-1.5, Y_LB, "W", C_COLUMBIA, GAP_B_BND),     # B gap bnd
    ...
    "SLIP": {
        ...
        "lbs": [
            (1.5, Y_LB, "M", C_COLUMBIA, GAP_B_FIELD),   # B gap field
            (-1.5, Y_LB, "W", C_COLUMBIA, GAP_A_BND),     # A gap bnd
            (7.5, Y_OLB, "B", C_COLUMBIA, FORCE_FIELD),   # coverage clean
            (-7.5, Y_OLB, "E", C_COLUMBIA, CONTAIN_BND),  # contain
        ],
        "secondary": _std_secondary(),
    },
}

# TE formation adjustment: B moves to LB depth field side
def get_front(front_key, has_te):
    """Return front dict, adjusting B to LB depth if TE present."""
    import copy
...
### Task 5: Stunt Data with Post-Stunt Gap Fits

**Files:**
- Modify: `/home/ksc4130/src/defensive_playbook/create_runfit_diagrams.py`

**Step 1: Define stunt data — movements AND post-stunt gap overrides**

Each stunt defines:
- `moves`: dict of player_label -> (dx, dy) for solid arrows
- `gap_overrides`: dict of player_label -> new gap_x (post-stunt fit changes)
- `legality`: which fronts it works on
...
STUNTS_4DOWN = {
    "SLANT": {
        ...
        "gap_overrides": {},  # T/N get arrows, others stay
        "legality": {
    ...
    "CRASH": {
        ...
        "gap_overrides": {
            "B": CONTAIN_FIELD,    # B replaces A as field contain
            "W": CONTAIN_BND,      # W replaces E as bnd contain
            "M": GAP_A_FIELD,      # M = free LB (A gap default)
        },
        "legality": {
            "SHADE": "BEST", "UNDER": "BEST", "EYES": "OK",
...
### Task 6: Core Rendering Functions

**Files:**
- Modify: `/home/ksc4130/src/defensive_playbook/create_runfit_diagrams.py`

**Step 1: Add the main render functions**
...
def draw_defense_runfit(ax, front_dict, stunt=None):
    ...
    for x, y, lbl, color, gap_x, group in all_players:
        ...
        if lbl in movers:
            # Solid arrow for stunt/blitz movement
            dx, dy = movers[lbl]
            draw_stunt_arrow(ax, x, y, x + dx, y + dy, color=C_RED)
        else:
            # Dotted line to gap fit
...
**Step 2: Test with a single diagram**

Add at bottom temporarily:
```python
if __name__ == "__main__":
    fig = render_front_diagram("SHADE", "2x2")
    save_fig(fig, "test_shade_2x2")
    print("Test diagram saved.")
```

Run: `cd /home/ksc4130/src/defensive_playbook && python3 create_runfit_diagrams.py`
Expected: "Test diagram saved." and PNG in /tmp/rv_runfit_diagrams/

**Step 3: Remove test code after verification**

---

### Task 7: Landscape DOCX Builder

**Files:**
- Modify: `/home/ksc4130/src/defensive_playbook/create_runfit_diagrams.py`

**Step 1: Add docx helper functions with landscape orientation**
...
### Task 8: Per-Front Document Generator

**Files:**
- Modify: `/home/ksc4130/src/defensive_playbook/create_runfit_diagrams.py`

**Step 1: Add the per-front docx generation function and main entry point**
...
def generate_front_docx(front_key):
    """Generate one landscape docx for a front: base vs 9 formations + stunts vs 2x2."""
    count = 0
    ...
    # Get legal stunts for this front
    legal_stunts = []
    ...
    # Section 1: Base alignments vs all formations
...
Run: `cd /home/ksc4130/src/defensive_playbook && python3 create_runfit_diagrams.py`
Expected: ~199 diagrams across 12 files in `diagram_docs/runfit/`

---

### Task 9: Visual Review and Corrections

**Step 1: Open output directory and spot-check diagrams**

Run: `explorer.exe "$(wslpath -w /home/ksc4130/src/defensive_playbook/diagram_docs/runfit)"`

**Step 2: Verify key diagrams:**
- SHADE vs 2×2: M dotted→A field, W dotted→B bnd, A/E dotted→contain, B dotted→force field
- CRASH on WIDE vs 2×2: A/E/T/N all have red solid arrows, B dotted→contain field, W dotted→contain bnd, M dotted→A gap
- GRIZZLY vs 21 pers: M at 10-tech, B/W as OLBs
- SLIP vs 2×2: B dotted→force (coverage clean), M→B field, W→A bnd
- ANGLE on SLIP vs 2×2: A+T+N all get red arrows

**Step 3: Fix any alignment or gap-fit issues found**
...
```bash
git add create_runfit_diagrams.py diagram_docs/runfit/ docs/plans/
git commit -m "feat: add run-fit diagram generator — 12 landscape docx files with gap responsibility lines for all fronts × formations × stunts"
```
